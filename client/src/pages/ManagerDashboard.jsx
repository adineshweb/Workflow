...
import { Search, RefreshCw, Hourglass, CheckCircle, XCircle, ClipboardCheck } from 'lucide-react';
...
const ManagerDashboard = () => {
  ...
  const pendingReview = requests.filter((r) => r.status === 'Submitted' || r.status === 'Reopened').length;
    ...
    },
    {
      ...
      accessor: (row) => (
        <RouterLink
          ...
          className="inline-flex items-center rounded-md bg-brand-50 px-2.5 py-1.5 text-xs font-semibold text-brand-700 shadow-xs hover:bg-brand-100 hover:text-brand-900 border border-brand-200 transition-all"
        >
          Review & Decision
        </RouterLink>
      ),
      ...
      <div className="space-y-6">
        ...
        >
          <div>
            ...
            <p className="text-sm text-slate-500">
              Review, approve, reject, or request clarification on employee submissions.
            </p>
          </div>
          <div>
        ...
        >
          <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs flex items-center gap-4">
            ...
            <div>
              ...
              <h4 className="text-2xl font-bold text-slate-900">{pendingReview}</h4>
            </div>
          </div>
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs flex items-center gap-4">
            ...
            <div>
              <p className="text-sm font-semibold text-slate-500">Total in System</p>
        ...
        >
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <h3 className="text-lg font-bold text-slate-950">Review Queue</h3>
              <p className="text-xs text-slate-400 mt-0.5">Toggle filters below to view specific states.</p>
            </div>
          ...
          <div className="overflow-x-auto">
            <Table
              ...
              emptyMessage="No requests in review queue matching current filters."
            />
          </div>
        </div>
      </div>
